...
int Wordlist::resize(int amt) {
	...
	if (newAllocated <= 0) {        //zeros or negative as invalid when size == 0
		if (size == 0) { return -2; }
		else { return -1; }
	}
	...
	for (int i = 0; i < size; i++) {
		if (list != nullptr && list[i] != nullptr) {
			newList[i] = list[i];   //copy items
			list[i] = nullptr;      
		}
		else {
			newList[i] = new char[20];       //still allocate something
			newList[i][0] = 0;
		}
	}
